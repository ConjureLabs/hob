...
function crawlDir(dir) {
  ...
  for (let resource of list) {
    let pathResolved = path.resolve(dir, resource)
    ...
    if (fileStat.isFile() && resource.length > 5 && resource.substr(-5) === '.styl') {
      prepareStylus(dir, pathResolved)
    }
  }
}

function prepareStylus(projectDir, filePath) {
...
module.exports = async function process({
  dir, // should be abs path to project root
  fresh = false
}) {
  if (fresh) {
  ...
  for (let subdir of subDirsToCrawl) {
    crawlDir(path.resolve(dir, subdir))
  }
}
